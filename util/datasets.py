...
def create_random_sample(dataset):
    labels = torch.Tensor([l for p, l in dataset.reader.samples]).long()
    class_weights = 1.0 / torch.bincount(labels)
    print(class_weights)
    sample_weights = torch.Tensor([class_weights[class_id] for class_id in labels])
